...
\item{4.0-3} (May 25, 2010): Testing Release for R 2.12

Extensive changes including:

	\begin{itemize}
		\item Added Roxygen-compliant in-source documentation
		\item {\tt demo(HelloWorld)} now produces an informative demo on creating a simple statistical package
		\item Removed the approximately half the models from Zelig
		\item Moved 22 models from the ``core'' package to more focused packages
		\item Added 'zelig.skeleton' method for rapidly creating Zelig packages
		\item Resolved error messages, warnings, and notes from R CMD check and R CMD build
		\item Moved 500+ page Zelig documentation
		\item Temporarily disabling bootstrapping from the {\tt sim} method
		\item Temporarily removing {\tt Average Treatment Effects} from the generalized linear models
	\end{itemize}
